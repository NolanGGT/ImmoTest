function extractMeilleursAgents() {
  try {
    // 1. __NEXT_DATA__
    ...
    if (nextDataEl) {
      ...
      const l = props?.listing || props?.annonce || props?.property || props?.ad
      if (l) {
        const prix = l.price || l.salePrice || l.prix
        const surface = l.area || l.surface || l.livingArea
        if (prix || surface) {
          return {
            ...
            ville: l.city || l.address?.city || l.location?.city || null,
            codePostal: l.postalCode || l.zipCode || l.address?.postalCode || null,
    ...
    console.error('[ImmoTest] MeilleursAgents:', e)
    return null
  }
}
